...
function import {
    if [ -f "$basedir/Dionysus/Dionysus-Server/src/main/java/$nms/$1.java" ]; then
        echo "ALREADY IMPORTED $1"
        return 0
    fi
    export importedmcdev="$importedmcdev $1"
    file="${1}.java"
    target="$basedir/Dionysus/Dionysus-Server/src/main/java/$nms/$file"
...
function importLibrary {
    ...
    for file in "$@"; do
        file="$prefix/$file"
        target="$basedir/Dionysus/Dionysus-Server/src/main/java/$file"
...
(
    cd Dionysus/Dionysus-Server/
    lastlog=$(git log -1 --oneline)
    if [[ "$lastlog" = *"ElainaSpigot-Extra mc-dev Imports"* ]]; then
        git reset --hard HEAD^
    fi
)
...
for f in $files; do
    ...
    if [ "$?" == "1" ]; then
        if [ ! -f "$basedir/Dionysus/Dionysus-Server/src/main/java/net/minecraft/server/$f.java" ]; then
            if [ ! -f "$decompiledir/$nms/$f.java" ]; then
                if [[ ! $f == ElainaSpigot* ]]; then
...
(
    ...
    rm -rf nms-patches
    git add src -A
    echo -e "ElainaSpigot-Extra mc-dev Imports\n\n$MODLOG" | git commit src -F -
    exit 0
)
